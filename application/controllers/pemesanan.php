...
class Pemesanan extends CI_Controller {
	...
	public function pending()
	{
		...
		if(!empty($cek))
		{
			...
			$tot_hal = $this->app_model->manualQuery("SELECT * from tbl_pesanan_header a left join tbl_pelanggan b on a.kode_pelanggan=b.kode_pelanggan where jenis='Pesanan'");
			...
			(select count(kode_pesanan) as jum from tbl_pesanan_detail where kode_pesanan=a.kode_pesanan) as jumlah from tbl_pesanan_header a left join tbl_pelanggan b 
			on a.kode_pelanggan=b.kode_pelanggan where a.jenis='Pesanan' and a.stts='pending' LIMIT ".$offset.","
			.$limit."");
			
			$this->load->view('global/bg_top',$d);
			$this->load->view('pemesanan/bg_home',$bc);
	...
	public function ok()
	{
		...
		if(!empty($cek))
		{
			...
			$tot_hal = $this->app_model->manualQuery("SELECT * from tbl_pesanan_header a left join tbl_pelanggan b on a.kode_pelanggan=b.kode_pelanggan where a.stts='Ok'");
			$config['base_url'] = base_url() . 'pemesanan/index/';
			$config['total_rows'] = $tot_hal->num_rows();
			$config['per_page'] = $limit;
			$config['uri_segment'] = 3;
			...
			$bc['dt_pesanan'] = $this->app_model->manualQuery("SELECT a.kode_pesanan, b.nama_pelanggan, a.tanggal_pesanan, a.stts, a.jenis,
			(select count(kode_pesanan) as jum from tbl_pesanan_detail where kode_pesanan=a.kode_pesanan) as jumlah from tbl_pesanan_header a left join tbl_pelanggan b 
			on a.kode_pelanggan=b.kode_pelanggan where a.stts='ok' LIMIT ".$offset.","
			.$limit."");
	...
	public function edit()
	{
		...
		{	
			...
			$bc['dt_pesanan_detail'] = $this->app_model->manualQuery("select a.kode_pesanan, a.kode_barang, a.qty, a.qty_terkirim, a.stts_pengiriman, 
			a.harga_tersimpan, b.nama_barang from tbl_pesanan_detail a left join tbl_barang b 
			on a.kode_barang=b.kode_barang where a.kode_pesanan='".$id['kode_pesanan']."'");
			...
			$this->load->view('global/bg_top',$d);
			$this->load->view('pemesanan/bg_edit_pemesanan',$bc);
			$this->load->view('global/bg_footer',$d);
		}
		else
		{
	...
	public function tambah()
	{
		...
		if(!empty($cek))
		{
			...
			$this->load->view('global/bg_top',$d);
			$this->load->view('pemesanan/bg_input_pemesanan',$bc);
			$this->load->view('global/bg_footer',$d);
		}
		else
		{
	...
	public function daftar_barang()
	{
		...
		if(!empty($cek))
		{
			...
			$this->load->view('pemesanan/bg_daftar_barang',$bc);
		}
		else
		{
			$st = $this->session->userdata('stts');
	...
	public function hapus()
	{
		...
		if(!empty($cek))
		{
			...
			?>
				<script> window.location = "<?php echo base_url(); ?>pemesanan"; </script>
			<?php
		}
		else
		{
	...
	public function update_pemesanan()
	{
		...
		if(!empty($cek))
		{
			...
			foreach($this->cart->contents() as $items)
			{
				...
				$d_detail['stts_pengiriman'] = "pending";
				if($d_detail['qty_terkirim']>=$items['qty']){
					$d_detail['stts_pengiriman'] = "terkirim";
				}
				$this->app_model->insertData("tbl_pesanan_detail",$d_detail);
				$d_u['stok'] = $this->app_model->getBalancedStok($d_detail['kode_barang'],$d_detail['qty']);
				$key['kode_barang'] = $d_detail['kode_barang'];
			...
			$this->session->unset_userdata('alert_edit');
			$this->session->unset_userdata('kd_pemesan');
			$this->session->unset_userdata('limit_add_cart');
			$this->cart->destroy();
			header('location:'.base_url().'pemesanan');
